...
public class TitleDrawFormat implements ITitleDrawFormat {
    ...
    public int measureWidth(Column column, TableConfig config) {
        FastTextDrawFormat format = null;
        ...
        int  widht = (int) (paint.measureText(maxStr));
        return  widht;
    }
    ...
    private void drawText(Canvas c, Column column, Rect rect, Paint paint) {
        ...
        DrawUtils.drawMultiText(c,paint,rect,column.getColumnName().split("\n"));
//        c.drawText(column.getColumnName(),
//                DrawUtils.getTextCenterX(rect.left,rect.right,paint),
//                DrawUtils.getTextCenterY((rect.bottom+rect.top)/2,paint) ,paint);
    }
